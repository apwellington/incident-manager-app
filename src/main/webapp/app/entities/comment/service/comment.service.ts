...
type RestOf<T extends IComment | NewComment> = Omit<T, 'commentDate'> & {
...
export class CommentService {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...comment,
      commentDate: comment.commentDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...restComment,
      commentDate: restComment.commentDate ? dayjs(restComment.commentDate) : undefined,
    };
  }
